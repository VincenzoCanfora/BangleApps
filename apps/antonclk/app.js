...

{
  ...
  // ----------------------------
  // Funzione per pubblicizzare i servizi BLE
  // ----------------------------
  function setupBLEAdvertising() {
    ...
    NRF.setServices({
      ...
      0x181A: { // Environmental Sensing
        ...
        // ----------------------------
        // AGGIUNTA: Caratteristica personalizzata per l'ACCELEROMETRO
        // ----------------------------
        0x2ACC: { // ID fittizio per esempio - Non è uno standard
          notify: true,
          readable: true,
          // Set di byte iniziale (6 byte: x,y,z con 2 byte per ciascuno)
          value: [0,0, 0,0, 0,0],
          onRead: () => acc_1 ? encodeAccel(acc_1) : [0,0, 0,0, 0,0]
        }
      },
  ...
  // ----------------------------
  // Funzione di aggiornamento dati BLE
  // ----------------------------
  function updateBLEData(hrm, bar, mag, gps, acc) {
    const services = {
      0x180D: { // Heart Rate
        0x2A37: {
      ...
      0x181A: {
        ...
        // ----------------------------
        // AGGIUNTA: Accelerometro
        // ----------------------------
        0x2ACC: {
          value: acc ? encodeAccel(acc) : [0,0, 0,0, 0,0],
          notify: true
        }
      },
      0x1819: {
  ...
  function encodeMag(data) {
    const x = toByteArray(Math.round(data.x), 2, true);
    const y = toByteArray(Math.round(data.y), 2, true);
    const z = toByteArray(Math.round(data.z), 2, true);
    return [x[0], x[1], y[0], y[1], z[0], z[1]];
  }

  // ----------------------------
  // Funzione per codificare l'accelerometro in 6 byte
  // (x, y, z a 2 byte ciascuno, segno incluso)
  // ----------------------------
  ...
  // Avvio dei servizi BLE
  setupBLEAdvertising();

  // Variabili globali che memorizzano gli ultimi dati ricevuti dai sensori
  ...
  Bangle.on("HRM", (hrm) => {
    hrm_1 = hrm;
    updateBLEData(hrm_1, bar_1, mag_1, gps_1, acc_1);
  });
  ...
  // ----------------------------
  // AGGIUNTA: Event listener per l'accelerometro
  // ----------------------------
  ...
  // L'accelerometro è solitamente già attivo di default;
  // se necessario, puoi impostare un polling interval:
  // Bangle.setPollInterval(80); // ms
}
